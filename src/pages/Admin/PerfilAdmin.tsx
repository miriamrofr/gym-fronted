...
export const PerfilAdmin = () => {
  ...
  const handleAceptar = handleSubmit(async (formData: any) => {
    ...
    try {
      ...
      url = `https://localhost:7245/api/admin/${user?.id}`;
  ...
  const getImageProfile = async () => {
    try {
      ...
      let url = `https://localhost:7245/api/admin/upload-image?idAdmin=${user?.id}`;
  ...
  const setInfoValue = () => {
    if (admin !== null) {
      ...
      setValue("calle", admin.calle);
      setValue("numero", admin.numero);
      setValue("provincia", admin.provincia);
      setValue("localidad", admin.localidad);
      setValue("codigoPostal", admin.codigoPostal);
    }
  };
  const getInfoAdmin = async () => {
    try {
      // Construir la URL con la búsqueda y paginación
      let url = `https://localhost:7245/api/admin/${user?.id}`;
